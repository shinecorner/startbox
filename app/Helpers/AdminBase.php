...
use App\Helpers\AdminPaginator;
use App\Helpers\Api;
 ...
 * @author Properos
 */
abstract class AdminBase extends AdminPaginator
{
    protected $model;
    protected $mode;
    protected $fillable;
    ...
    public function buildingModel($model, $options = [])
    {
        ...
        if (isset($options['q']) && is_string($options['q']) && $options['q'] != '') {
            ...
            if (is_array($fields) && count($fields) > 0) {
                ...
                \Log::info(str_replace("?field?", "CONCAT_WS('|'," . implode(",", $fields) . ")", $q));
                ...
                case 'query':
                    if (is_string($value)) {
                        if(Helper::isJson($value)){
                            $q = json_decode($value, true);
                            if (is_array($q) && Helper::isAssoc($q)) {
                                $options['q'] = \Arr::get($q, 'value', '');
                                $options['q_fields'] = \Arr::get($q, 'fields', []);
                            }
                        }else{
                            $options['q'] = $value;
                        }
